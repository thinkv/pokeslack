...
class Pokeslack:
    ...
    def try_send_pokemon(self, pokemon, position, distance, debug):
        ...
        travel_time = padded_distance / 1.38889 # assumes 5kph (or 1.38889 meter per second) walking speed
        ...
        message = 'I found a <%s|%s> %s <%s|%s meteres away> from the Melbourne office expiring in %s%s' % (pokedex_url, pokemon['name'], stars, map_url, meters_away, time_remaining, from_lure)
